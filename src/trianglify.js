...
import { memoCounter } from "./memoize";
...
export default function trianglify(_opts = {}) {
  ...
  const yColors =
    opts.yColors === "match" ? xColors : processColorOpts(opts.yColors);
  ...
  const pattern = new Pattern(points, polys, opts);

  console.log("memoizations:", memoCounter);
  return pattern;
}
